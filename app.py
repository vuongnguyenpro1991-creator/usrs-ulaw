...
def call_gemini(topic):
    try:
        ...
        prompt = f"""Bạn là một Chuyên gia Thư viện và Luật sư giỏi tại Đại học Luật TP.HCM. Nhiệm vụ: Phân tích đề tài nghiên cứu sau: "{topic}". Trả về ĐÚNG JSON: {{"chuyen_nganh": ["..."], "tu_khoa_vn": ["..."], "tu_khoa_en": ["..."], "tu_khoa_dac_thu": ["..."]}} không giải thích thêm."""
...
with st.sidebar:
    ...
    with col_logo2:
        try:
            ...
            pass # Nếu bạn chưa kịp tải logo lên thì hệ thống không báo lỗi
    ...
    with st.container(border=True):
        ...
        with cb1:
            ...
            if st.button("🔄 Khôi phục", use_container_width=True):
                st.session_state.ai_data = {"chuyen_nganh": ["Đang chờ AI phân tích..."], "tu_khoa_vn": ["Đang chờ AI phân tích..."], "tu_khoa_en": ["Đang chờ AI phân tích..."], "tu_khoa_dac_thu": ["Đang chờ AI phân tích..."]}
                st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
        with cb2:
            st.markdown("<div class='btn-blue'>", unsafe_allow_html=True)
