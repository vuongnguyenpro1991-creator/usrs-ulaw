...
def call_gemini(topic):
    try:
        ...
        # ĐÃ CẬP NHẬT PROMPT: Ép AI bỏ qua các từ khóa học thuật chung chung, chỉ lấy "Thực thể pháp lý lõi"
...
1. Chỉ bóc tách 1 đến 3 TỪ KHÓA LÕI là thực thể pháp lý. 
2. TUYỆT ĐỐI KHÔNG đưa vào các từ ngữ chung chung như: "thực tiễn", "đường lối", "giải quyết", "quy định", "pháp luật", "bất cập", "hoàn thiện", "một số vấn đề".
3. Kiểm tra xem đề tài có liên quan đến pháp luật nước ngoài hay đạo luật đặc thù quốc tế (như CISG, FDCPA...) không.
...
# 2. TÙY CHỈNH CSS CHUYÊN SÂU (Giữ nguyên toàn bộ)
st.markdown("""
<style>
    ...
    .query-preview-box {
        ...
        border-left: 4px solid #38bdf8; overflow-x: auto; line-height: 1.6;
    }
    .query-label {font-size: 12px; font-weight: 700; color: #94a3b8; text-transform: uppercase; margin-bottom: 4px; display: block;}
...
# ĐÃ CHỈNH SỬA CỘT 2: LOGIC HIỂN THỊ CHUỖI TRUY VẤN
with col2:
    with st.container(border=True):
        ...
        if sel_vn and sel_vn[0] != "Đang chờ AI phân tích...":
            vn_queries = []
            vn_queries.append("<span class='query-label'>🔸 Tìm đơn lẻ:</span>")
            for kw in sel_vn:
                vn_queries.append(f'"{kw}"')
            if len(sel_vn) > 1:
        ...
        if sel_en and sel_en[0] != "Đang chờ AI phân tích...":
            en_queries = []
            en_queries.append("<span class='query-label'>🔸 Tìm đơn lẻ:</span>")
            for kw in sel_en:
                en_queries.append(f'"{kw}"')
            if len(sel_en) > 1:
        ...
        if ai.get("co_yeu_to_nuoc_ngoai", False) and ai.get("tu_khoa_dac_thu", []) and ai["tu_khoa_dac_thu"][0] != "Đang chờ AI phân tích...":
            ...
            if sel_dac_thu:
                dt_queries = []
                dt_queries.append("<span class='query-label'>🔸 Tìm đơn lẻ:</span>")
                for kw in sel_dac_thu:
                    dt_queries.append(f'"{kw}"')
                st.markdown(f"<div class='query-preview-box'>{'<br>'.join(dt_queries)}</div>", unsafe_allow_html=True)

        # Lệnh kết hợp đặc biệt (Chỉ nối Tiếng Việt với Luật Đặc thù nếu có. Đã xóa logic nối Việt-Anh)
